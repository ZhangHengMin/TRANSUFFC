function [Z,iter, stopC,Time ]=SpULRR23(X, lambda, ranks, mu, tol, maxIter,term_obj)
% 2022.4.6 update
% min lambda*1/3(\|U\|_F^2+2\|V\|_*)+|E|_2,p
...
for ii = 1:2
    if ii == 1
        M{ii} = randn(size(X, ii), ranks);              % fix
        %M{ii} = randn(size(D, ii), ranks);
        [U{ii}, aa1] = qr(X'*M{ii}, 0);
    else
        M{ii} = randn(size(X, 3-ii), ranks);            % fix
...
for iter=1:maxIter
    ...
    %-----------upadat U{1}=U, U{2}=V----------------%
    u22=U{2}'*U{2};
    U{1}=Z*U{2}/(2/3*Lbeta*eye(ranks)+u22);%
    
    W2 = 1/2*(u22+mValue)^(-1/2);
    U{2} = Z'*U{1}/(4/3*Lbeta*W2+U{1}'*U{1});
    
    %-------------update Z----------------%
    M1=U{1}*U{2}';M2=X-b*ones(1,ncr(1,2))-E;
    Z=XXEye\(M1+X'*M2);
